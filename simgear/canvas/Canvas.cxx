...
/**
   ...
   */
class CanvasImageCallback : public vsg::Camera::DrawCallback
{
public:
    ...
    virtual void operator()(osg::RenderInfo& renderInfo) const
    {
        ...
        if (hasSubscribers()) {
            ...
            vsg::ref_ptr<vsg::Image> image = new vsg::Image;
            ...
            vsg::ref_ptr<vsg::Image> image_rgb = ImageUtils::convertToRGB8(image);
            {
...
//----------------------------------------------------------------------------
Canvas::CullCallback::CullCallback(const CanvasWeakPtr& canvas) : _canvas(canvas)
{
}

//----------------------------------------------------------------------------
void Canvas::CullCallback::operator()(vsg::Node* node,
                                      osg::NodeVisitor* nv)
{
...
//----------------------------------------------------------------------------
Canvas::Canvas(SGPropertyNode* node) : PropertyBasedElement(node),
...
void Canvas::update(double delta_time_sec)
{
    ...
    if (_status & STATUS_DIRTY) {
        ...
        if (!_texture.serviceable()) {
            ...
            vsg::Camera* camera = _texture.getCamera();

            // XXX: Render order is determined by the Canvas index
            int order = _node->getIndex();
            camera->setRenderOrder(vsg::Camera::PRE_RENDER, order);
...
void Canvas::valueChanged(SGPropertyNode* node)
{
    ...
    if (strutils::starts_with(name, "status") || strutils::starts_with(name, "data-"))
        return;
    _render_dirty = true;

    bool handled = true;
    if (node->getParent()->getParent() == _node && node->getParent()->getNameString() == "placement") {
        ...
        if (index < _placements.size()) {
            ...
            if (!placements.empty()) {
                ...
                for (auto& placement : placements) {
                    ...
                    if (placement->getProps() == node->getParent() && !placement->childChanged(node))
...
//----------------------------------------------------------------------------
vsg::Camera* Canvas::getCamera() const
{
