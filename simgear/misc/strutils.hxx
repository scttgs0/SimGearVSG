...
// [lr]find - find first left/right occurrence of a single character, or one of characters from a set
...
/**
         ...
         */
string_list split_on_any_of(const std::string&, const char* separators);
...
/**
         * Read a time value, separated by colons, as a value in hours.
...
/**
      ...
      * malformed
      */
void decodeBase64(const std::string& a, UInt8Vector& output);

/**
      * convert bytes to hexadecimal equivalent
      @param sep Optional separator character, such as ':' or ' '
      */
std::string encodeHex(const std::string& bytes, char sep = 0);
...
inline std::string unescape(const std::string& str)
{
    ...
    */
    ...
    /**
       ...
       *
       * Supported formats:
       *   <signed decimal degrees latitude>,<signed decimal degrees longitude>
       *   <unsigned decimal degrees>[NS],<unsigned decimal degrees>[EW]
       *   <degrees>*<decimal minutes>'[NS],<degrees>*<decimal minutes>'[EW]
       *
       * Latitude and longitude are parsed separately so the formats for each
