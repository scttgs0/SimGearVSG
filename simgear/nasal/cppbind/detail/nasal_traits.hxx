...
#define SG_MAKE_TRAIT(templ, type, attr)                       \
    template templ struct attr<type> : public std::true_type { \
    };

SG_MAKE_TRAIT(<class T>, SGVec2<T>, is_vec2)
SG_MAKE_TRAIT(<>, vsg::bvec2, is_vec2)
SG_MAKE_TRAIT(<>, vsg::dvec2, is_vec2)
SG_MAKE_TRAIT(<>, vsg::vec2, is_vec2)
SG_MAKE_TRAIT(<>, vsg::svec2, is_vec2)
SG_MAKE_TRAIT(<>, vsg::ivec2, is_vec2)

SG_MAKE_TRAIT(<class T>, SGVec4<T>, is_vec4)
SG_MAKE_TRAIT(<>, vsg::dvec4, is_vec4)
SG_MAKE_TRAIT(<>, vsg::vec4, is_vec4)
...
SG_MAKE_SHARED_PTR_TRAIT(vsg::ref_ptr, osg::observer_ptr, true)
...
template <class T>
struct supports_weak_ref<SGSharedPtr<T>> : public std::bool_constant<std::is_base_of<SGWeakReferenced, T>::value> {
};
...
struct shared_ptr_storage {
    ...
    template <class U>
    static std::enable_if_t<std::is_same<U, element_type*>::value, element_type*>
    ...
    static std::enable_if_t<
        std::is_same<U, strong_ref>::value || (std::is_same<U, weak_ref>::value && supports_weak_ref<U>::value),
        U>
    get(storage_type* ptr)
    {
        return U(*ptr);
...
template <class T>
struct shared_ptr_storage<SGSharedPtr<T>> : public internal::intrusive_ptr_storage<SGSharedPtr<T>> {
    using storage_type = T;
    using element_type = T;
...
template <class T>
struct shared_ptr_storage<vsg::ref_ptr<T>> : public internal::intrusive_ptr_storage<vsg::ref_ptr<T>> {
    using storage_type = T;
    using element_type = T;
