...
public:
    ...
    // Members
    //
    vsg::vec3Array* getVertices(void)
    {
        return vertices;
    }

    vsg::vec3Array* getNormals(void)
    {
        return normals;
    }
    ...
    ::std::vector<vsg::vec3> pdir1, pdir2;
    ::std::vector<float> curv1, curv2;
    ::std::vector<vsg::vec4> dcurv;
    ::std::vector<vsg::vec3> cornerareas;
    ...
    inline vsg::vec3 trinorm(const vsg::vec3& v0, const vsg::vec3& v1, const vsg::vec3& v2)
    {
        return ((v1 - v0) ^ (v2 - v0)) * 0.5;
    }

    static inline const float angle(const vsg::vec3& v1, const vsg::vec3& v2)
    {
...
public:
    ...
    float cornerangle(int i, int j)
    {
        ...
        if (faces.empty()) need_faces();
        const vsg::vec3& p0 = (*vertices)[faces[i][j]];
        const vsg::vec3& p1 = (*vertices)[faces[i][(j + 1) % 3]];
        const vsg::vec3& p2 = (*vertices)[faces[i][(j + 2) % 3]];
    ...
    float dihedral(int i, int j)
    {
        ...
        vsg::vec3 mynorm = trinorm(i);
        vsg::vec3 othernorm = trinorm(across_edge[i][j]);
        float ang = angle(mynorm, othernorm);
        vsg::vec3 towards = (((*vertices)[faces[i][(j + 1) % 3]] + (*vertices)[faces[i][(j + 2) % 3]]) - (*vertices)[faces[i][j]]) * 0.5f;
