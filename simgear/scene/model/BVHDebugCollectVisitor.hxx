...
{
public:
    BVHDebugCollectVisitor(const double& time, unsigned level = ~0u) : _group(new vsg::Group),
                                                                       _time(time),
                                                                       _level(level),
                                                                       _currentLevel(0)
    {
    ...
    {
        addNodeSphere(node);
        vsg::ref_ptr<vsg::Group> oldGroup = _group;
        vsg::ref_ptr<osg::MatrixTransform> transform = new osg::MatrixTransform;
        transform->setMatrix(vsg::mat4(node.getToWorldTransform().data()));
        _group = transform;
        ++_currentLevel;
        node.traverse(*this);
        --_currentLevel;
    ...
    virtual void apply(BVHMotionTransform& node)
    {
        ...
        vsg::ref_ptr<vsg::Group> oldGroup = _group;
        vsg::ref_ptr<osg::MatrixTransform> transform = new osg::MatrixTransform;
        transform->setMatrix(vsg::mat4(node.getToWorldTransform(_time).data()));
        _group = transform;
        ++_currentLevel;
        node.traverse(*this);
        --_currentLevel;
    ...
    {
        addNodeBox(node, data);
        addTriangle(node.getTriangle(data), vsg::vec4(0.5, 0, 0.5, 0.2));
    }

    vsg::Node* getNode() const { return _group.get(); }

    static unsigned allLevels() { return ~0u; }
    static unsigned triangles() { return ~0u - 1; }

private:
    void addTriangle(const SGTrianglef& triangle, const vsg::vec4& color)
    {
        if (_level != triangles())
            return;
    ...
    void addNodeSphere(const BVHNode& node)
    {
        ...
        addShape(shape, vsg::vec4(0.5f, 0.5f, 0.5f, 0.1f));
    }
    ...
    void addNodeBox(const BVHStaticNode& node, const BVHStaticData& data)
    {
        ...
        addShape(shape, vsg::vec4(0.5f, 0, 0, 0.1f));
    }

    void addShape(osg::Shape* shape, const vsg::vec4& color)
    {
