...
/**
   ...
   */
class Element : public PropertyBasedElement
{
public:
    ...
    struct StyleSetter : public SGReferenced {
    ...
    /**
       ...
       */
    virtual bool hitBound(const vsg::vec2& global_pos,
    ...
    /**
       ...
       */
    vsg::vec2 posToLocal(const vsg::vec2& pos) const;

    /**
       * @brief transform position from local coordinates
       * 
       * @param pos - in coordinates of this element
       * @return vsg::vec2  in coordinates of parent Group
       */
    vsg::vec2 posFromLocal(const vsg::vec2& pos) const;

    /**
       * Transform from coordinates of our Canvas, to this
       * Element's local coordinates
      */
    vsg::vec2 canvasToLocal(const vsg::vec2& pos) const;

    /**
       * Transform from our local coordinates into those
       * of our Canvas
      */
    vsg::vec2 localToCanvas(const vsg::vec2& pos) const;
    ...
    /**
       ...
       */
    virtual osg::BoundingBox getTransformedBounds(const vsg::mat4& m) const;

    /**
       * Get the transformation matrix (product of all transforms)
       */
    vsg::mat4 getMatrix() const;
...
protected:
    ...
    static std::function<void(Derived&, T)>
    ...
    static StyleSetterFuncUnchecked
...
private:
    vsg::ref_ptr<osg::Drawable> _drawable;
...
struct enum_traits<canvas::Element::ReferenceFrame> {
    ...
    {
        return frame >= canvas::Element::GLOBAL && frame <= canvas::Element::LOCAL;
    }
};
