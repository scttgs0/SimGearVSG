...
EventVisitor::EventVisitor(TraverseMode mode,
                           const vsg::vec2& pos,
                           const ElementPtr& root) : _traverse_mode(mode),
                                                     _root(root)
{
...
bool EventVisitor::apply(Element& el)
{
    ...
    if (_traverse_mode == TRAVERSE_DOWN) {
        const vsg::vec2& pos = _target_path.back().local_pos;
        const vsg::vec2 local_pos = el.posToLocal(pos);
        ...
        if (_root.get() != &el && !el.hitBound(_target_path.front().local_pos, pos, local_pos))
            return false;
