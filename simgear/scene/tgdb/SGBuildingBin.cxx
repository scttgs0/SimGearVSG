...
struct MakeBuildingLeaf {
    MakeBuildingLeaf(float range, Effect* effect, bool fade) : _range(range), _effect(effect), _fade_out(fade) {}

    MakeBuildingLeaf(const MakeBuildingLeaf& rhs) : _range(rhs._range), _effect(rhs._effect), _fade_out(rhs._fade_out)
    {
    }
    ...
    LOD* operator()() const
    {
        ...
        // Front face
        v->push_back(vsg::vec3(0.0, -0.5, -1.0)); // bottom right
        v->push_back(vsg::vec3(0.0, 0.5, -1.0));  // bottom left
        v->push_back(vsg::vec3(0.0, 0.5, 0.0));   // top left

        v->push_back(vsg::vec3(0.0, 0.5, 0.0));   // top left
        v->push_back(vsg::vec3(0.0, -0.5, 0.0));  // top right
        v->push_back(vsg::vec3(0.0, -0.5, -1.0)); // bottom right

        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(1, 0, 0));    // normal
            c->push_back(vsg::vec4(1, 0, 0, 0)); // color - used to differentiate wall from roof
            t->push_back(vsg::vec2(0.0, 0.0));
        }

        // Left face
        v->push_back(vsg::vec3(-1.0, -0.5, -1.0)); // bottom right
        v->push_back(vsg::vec3(0.0, -0.5, -1.0));  // bottom left
        v->push_back(vsg::vec3(0.0, -0.5, 0.0));   // top left

        v->push_back(vsg::vec3(0.0, -0.5, 0.0));   // top left
        v->push_back(vsg::vec3(-1.0, -0.5, 0.0));  // top right
        v->push_back(vsg::vec3(-1.0, -0.5, -1.0)); // bottom right

        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(0, -1, 0));   // normal
            c->push_back(vsg::vec4(1, 0, 0, 0)); // color - used to differentiate wall from roof
            t->push_back(vsg::vec2(0.0, 0.0));
        }
        ...
        // Back face
        v->push_back(vsg::vec3(-1.0, 0.5, -1.0));  // bottom right
        v->push_back(vsg::vec3(-1.0, -0.5, -1.0)); // bottom left
        v->push_back(vsg::vec3(-1.0, -0.5, 0.0));  // top left
        ...
        // MAIN BODY
        // Front face
        v->push_back(vsg::vec3(0.0, -0.5, 0.0)); // bottom right
        v->push_back(vsg::vec3(0.0, 0.5, 0.0));  // bottom left
        v->push_back(vsg::vec3(0.0, 0.5, 1.0));  // top left

        v->push_back(vsg::vec3(0.0, 0.5, 1.0));  // top left
        v->push_back(vsg::vec3(0.0, -0.5, 1.0)); // top right
        v->push_back(vsg::vec3(0.0, -0.5, 0.0)); // bottom right

        t->push_back(vsg::vec2(1.0, 0.0)); // bottom right
        t->push_back(vsg::vec2(0.0, 0.0)); // bottom left
        t->push_back(vsg::vec2(0.0, 1.0)); // top left

        t->push_back(vsg::vec2(0.0, 1.0)); // top left
        t->push_back(vsg::vec2(1.0, 1.0)); // top right
        t->push_back(vsg::vec2(1.0, 0.0)); // bottom right
        ...
        // Left face
        v->push_back(vsg::vec3(-1.0, -0.5, 0.0)); // bottom right
        v->push_back(vsg::vec3(0.0, -0.5, 0.0));  // bottom left
        v->push_back(vsg::vec3(0.0, -0.5, 1.0));  // top left

        v->push_back(vsg::vec3(0.0, -0.5, 1.0));  // top left
        v->push_back(vsg::vec3(-1.0, -0.5, 1.0)); // top right
        v->push_back(vsg::vec3(-1.0, -0.5, 0.0)); // bottom right

        t->push_back(vsg::vec2(0.0, 0.0)); // bottom right
        t->push_back(vsg::vec2(1.0, 0.0)); // bottom left
        t->push_back(vsg::vec2(1.0, 1.0)); // top left

        t->push_back(vsg::vec2(1.0, 1.0)); // top left
        t->push_back(vsg::vec2(0.0, 1.0)); // top right
        t->push_back(vsg::vec2(0.0, 0.0)); // bottom right

        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(0, -1, 0));   // normal
            c->push_back(vsg::vec4(0, 0, 0, 1)); // color - used to differentiate wall from roof
        }

        // Back face
        v->push_back(vsg::vec3(-1.0, 0.5, 0.0));  // bottom right
        v->push_back(vsg::vec3(-1.0, -0.5, 0.0)); // bottom left
        v->push_back(vsg::vec3(-1.0, -0.5, 1.0)); // top left
        ...
        c->push_back(vsg::vec4(0, 1, 0, 0)); // color - used to differentiate wall from roof
        c->push_back(vsg::vec4(0, 1, 0, 0)); // color - used to differentiate wall from roof
        c->push_back(vsg::vec4(0, 1, 1, 0)); // color - used to differentiate wall from roof
        ...
        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(0.707, 0, 0.707)); // normal
        }
        ...
        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(0, -0.707, 0.707)); // normal
        }
        ...
        for (int i = 0; i < 6; ++i) {
            n->push_back(vsg::vec3(0, 0, 1.0)); // normal
        }
        ...
        geom->setVertexAttribArray(BUILDING_POSITION_ATTR, new vsg::vec3Array, Array::BIND_PER_VERTEX, 1);
        geom->setVertexAttribArray(BUILDING_SCALE_ATTR, new vsg::vec3Array, Array::BIND_PER_VERTEX, 1);
        geom->setVertexAttribArray(BUILDING_ATTR1, new vsg::vec3Array, Array::BIND_PER_VERTEX, 1);
        geom->setVertexAttribArray(BUILDING_ATTR2, new vsg::vec3Array, Array::BIND_PER_VERTEX, 1);
...
{
    return floor(a * pack_precision + 0.5) + floor(b * pack_precision + 0.5) * pack_precision1 + floor(c * pack_precision + 0.5) * pack_precision1 * pack_precision1;
};
...
struct AddBuildingLeafObject {
    void operator()(LOD* lod, const SGBuildingBin::BuildingInstance building) const
    {
        ...
        vsg::vec3Array* positions = static_cast<vsg::vec3Array*>(geom->getVertexAttribArray(BUILDING_POSITION_ATTR)); // (x,y,z)
        vsg::vec3Array* scale = static_cast<vsg::vec3Array*>(geom->getVertexAttribArray(BUILDING_SCALE_ATTR));        // (width, depth, height)
        vsg::vec3Array* attrib1 = static_cast<vsg::vec3Array*>(geom->getVertexAttribArray(BUILDING_ATTR1));
        vsg::vec3Array* attrib2 = static_cast<vsg::vec3Array*>(geom->getVertexAttribArray(BUILDING_ATTR2));

        positions->push_back(building.position);
        // Depth is the x-axis, width is the y-axis
        scale->push_back(vsg::vec3(building.depth, building.width, building.height));
        attrib1->push_back(vsg::dvec3(
...
// positions.
SGBuildingBin::SGBuildingBin(const SGPath& absoluteFileName, const SGMaterial* mat) : SGBuildingBin::SGBuildingBin(mat)
{
...
ref_ptr<Group> SGBuildingBin::createBuildingsGroup(Matrix transInv, const SGReaderWriterOptions* options)
{
    ...
    ref_ptr<Group> group = new vsg::Group();
    ...
    for (size_t j = 0; j < quadbuilding.getRoot()->getNumChildren(); ++j) {
        vsg::Node* c = quadbuilding.getRoot()->getChild(j);
...
vsg::Group* createRandomBuildings(SGBuildingBinList& buildings, const vsg::mat4& transform,
