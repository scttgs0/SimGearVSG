...
void SGGeodesy::SGCartToGeod(const SGVec3<double>& cart, SGGeod& geod)
{
    // according to
    // H. Vermeille,
...
void SGGeodesy::SGGeodToCart(const SGGeod& geod, SGVec3<double>& cart)
{
    // according to
    // H. Vermeille,
    // Direct transformation from geocentric to geodetic coordinates,
...
void SGGeodesy::SGCartToGeoc(const SGVec3<double>& cart, SGGeoc& geoc)
{
...
void SGGeodesy::SGGeocToCart(const SGGeoc& geoc, SGVec3<double>& cart)
{
...
{
    //double e4 = e2*e2;
    return SGMiscd::pi() * 0.5 * (1.0 - e2 * (1.0 / 4.0 + e2 * (3.0 / 64.0 + e2 * (5.0 / 256.0))));
}
    ...
    if (fabs(s) < 0.01) { // distance < centimeter => congruency
        ...
        *az2 = 180.0 + az1;
        ...
        do {
            ...
            temp = sig;
            sig = first +
                  tb * sinsig * (c2sigm + tb * (cossig * (-1.0 + 2.0 * c2sigm * c2sigm) - tb * c2sigm * (-3.0 + 4.0 * sinsig * sinsig) * (-3.0 + 4.0 * c2sigm * c2sigm) / 6.0) / 4.0);
...
bool SGGeodesy::direct(const SGGeod& p1, double course1,
                               ...
                               double* s)
{
    ...
    if ((fabs(fabs(lat1) - 90.0) < 200 * testv) &&
        ...
        *s = s1 + s2;
        ...
        sinu2 = temp * cosu2;
        // Safeguard to catch formula of Vincenty not converging
        ...
        do {
            ...
            c2sigm = (sinu1 == 0.0 || sinu2 == 0.0 ? cossig : cossig - 2.0 * sinu1 * sinu2 / cos2saz);
            tc = f * cos2saz * (4.0 + f * (4.0 - 3.0 * cos2saz)) / 16.0;
            temp = dlams;
            ...
            if (++iterations > MAX_ITERATIONS) {
                throw sg_exception(std::string("geo_inverse_wgs_84 won't converge MAX_ITERATIONS reached "));
            }
...
bool SGGeodesy::inverse(const SGGeod& p1, const SGGeod& p2, double& course1,
...
void SGGeodesy::advanceRadM(const SGGeoc& geoc, double course, double distance,
                            SGGeoc& result)
{
...
bool SGGeodesy::radialIntersection(const SGGeoc& a, double r1,
...
bool SGGeodesy::radialIntersection(const SGGeod& a, double aRadial,
