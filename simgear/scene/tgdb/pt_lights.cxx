...
namespace {
typedef std::tuple<float, vsg::vec3, float, float, bool> PointParams;
...
{
    if (useTriangles) {
        vsg::vec3Array* vertices = new vsg::vec3Array(3);
...
SGLightFactory::getSequenced(const SGDirectionalLightBin& lights, const SGReaderWriterOptions* options)
{
    ...
    Effect* effect = getLightEffect(24.0f, vsg::vec3(1.0, 0.0001, 0.000001),
    ...
    sequence->addChild(new vsg::Group, 1.9 + (0.1 * sg_random()) - (lights.getNumLights() * flashTime));
...
SGLightFactory::getReil(const SGDirectionalLightBin& lights, const SGReaderWriterOptions* options)
{
    ...
    sequence->addChild(egeode, flashTime);
    sequence->addChild(new vsg::Group, 1.9 + 0.1 * sg_random() - flashTime);
    sequence->setInterval(osg::Sequence::LOOP, 0, -1);
    sequence->setDuration(1.0f, -1);
    sequence->setMode(osg::Sequence::START);
    sequence->setSync(true);
...
SGLightFactory::getOdal(const SGLightBin& lights, const SGReaderWriterOptions* options)
{
    ...
    osg::Sequence* sequence = new osg::Sequence;
    sequence->setDefaultTime(flashTime);
    Effect* effect = getLightEffect(20.0f, vsg::vec3(1.0, 0.0001, 0.000001),
    ...
    sequence->addChild(new vsg::Group, 4 * flashTime);
    // runway end lights
    EffectGeode* egeode = new EffectGeode;
    egeode->setEffect(effect);
    ...
    sequence->addChild(new vsg::Group, 1.9 + (0.1 * sg_random()) - ((lights.getNumLights() + 2) * flashTime));
    sequence->setInterval(osg::Sequence::LOOP, 0, -1);
    sequence->setDuration(1.0f, -1);
    sequence->setMode(osg::Sequence::START);
    sequence->setSync(true);
...
SGLightFactory::getHoldShort(const SGDirectionalLightBin& lights, const SGReaderWriterOptions* options)
{
    ...
    sequence->addChild(new vsg::Group, 0.2);
    // ...and increase the lights in steps
    for (int i = 0; i < 5; i++) {
        Effect* effect = getLightEffect(12.0f + i, vsg::vec3(1, 0.001, 0.0002),
...
SGLightFactory::getGuard(const SGDirectionalLightBin& lights, const SGReaderWriterOptions* options)
{
    ...
    osg::Sequence* sequence = new osg::Sequence;
    sequence->setDefaultTime(flashTime);
    Effect* effect = getLightEffect(16.0f, vsg::vec3(1.0, 0.001, 0.0002),
