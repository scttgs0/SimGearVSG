...
public:
    /**
        * Clones an image.
        *
        * Use this instead of the vsg::Image copy construtor, which keeps the referenced to
        ...
        */
    static vsg::Image* cloneImage(const vsg::Image* image);
    ...
    /**
        ...
        */
    static void fixInternalFormat(vsg::Image* image);
    ...
    /**
        ...
        * values instead.
        */
    static void markAsUnNormalized(vsg::Image* image, bool value);

    /** Inverse of above. */
    static void markAsNormalized(vsg::Image* image, bool value) { markAsUnNormalized(image, !value); }

    /**
        * Whether the image has been marked as containing un-normalized values.
        */
    static bool isUnNormalized(const vsg::Image* image);

    /**
        * Whether the image has been marked as containing normalized values.
        */
    static bool isNormalized(const vsg::Image* image) { return !isUnNormalized(image); }
    ...
    /**
        ...
        */
    static vsg::Image* cropImage(
        const vsg::Image* image,
    ...
    /**
        ...
        */
    static vsg::Image* createMipmapBlendedImage(
        const vsg::Image* primary,
        const vsg::Image* secondary);

    /**
        * Creates a new image containing mipmaps built with nearest-neighbor
        * sampling.
        */
    static vsg::Image* buildNearestNeighborMipmaps(
        const vsg::Image* image);
    ...
    /**
        ...
        */
    static bool mix(vsg::Image* dest, const vsg::Image* src, float a);
    ...
    /**
        ...
        */
    static vsg::Image* createSharpenedImage(const vsg::Image* image);
    ...
    /**
        ...
        */
    static bool flattenImage(vsg::Image* image, std::vector<vsg::ref_ptr<vsg::Image>>& output);

    /**
        * Gets whether the input image's dimensions are powers of 2.
        */
    static bool isPowerOfTwo(const vsg::Image* image);

    /**
        * Gets a transparent, single pixel image used for a placeholder
        */
    static vsg::Image* createEmptyImage();

    /**
        * Gets a transparent image used for a placeholder with the specified dimensions
        */
    static vsg::Image* createEmptyImage(unsigned int s, unsigned int t);

    /**
        * Creates a one-pixel image.
        */
    static vsg::Image* createOnePixelImage(const vsg::vec4& color);

    /**
        * Tests an image to see whether it's "empty", i.e. completely transparent,
        * within an alpha threshold.
        */
    static bool isEmptyImage(const vsg::Image* image, float alphaThreshold = 0.01);
    ...
    /**
        ...
        */
    static bool isSingleColorImage(const vsg::Image* image, float threshold = 0.01);

    /**
        * Returns true if it is possible to convert the image to the specified
        * format/datatype specification.
        */
    static bool canConvert(const vsg::Image* image, GLenum pixelFormat, GLenum dataType);

    /**
        * Converts an image to the specified format.
        */
    static vsg::Image* convert(const vsg::Image* image, GLenum pixelFormat, GLenum dataType);

    /**
        *Converts the given image to RGB8
        */
    static vsg::Image* convertToRGB8(const vsg::Image* image);

    /**
        *Converts the given image to RGBA8
        */
    static vsg::Image* convertToRGBA8(const vsg::Image* image);

    /**
        * True if the two images are of the same format (pixel format, data type, etc.)
        * though not necessarily the same size, depth, etc.
        */
    static bool sameFormat(const vsg::Image* lhs, const vsg::Image* rhs);

    /**
        * True if the two images have the same format AND size, and can therefore
        * be used together in a texture array.
        */
    static bool textureArrayCompatible(const vsg::Image* lhs, const vsg::Image* rhs);

    /**
        *Compares the image data of two images and determines if they are equivalent
        */
    static bool areEquivalent(const vsg::Image* lhs, const vsg::Image* rhs);

    /**
        * Whether two colors are roughly equivalent.
        */
    static bool areRGBEquivalent(const vsg::vec4& lhs, const vsg::vec4& rhs, float epsilon = 0.01f)
    {
        return fabs(lhs.r() - rhs.r()) < epsilon &&
    ...
    /**
        ...
        */
    static bool hasAlphaChannel(const vsg::Image* image);
    ...
    /**
        ...
        */
    static bool hasTransparency(const vsg::Image* image, float alphaThreshold = 1.0f);
    ...
    /**
        ...
        */
    static bool featherAlphaRegions(vsg::Image* image, float maxAlpha = 0.0f);
    ...
    /**
        ...
        */
    static bool convertToPremultipliedAlpha(vsg::Image* image);

    /**
        * Checks whether the given image is compressed
        */
    static bool isCompressed(const vsg::Image* image);

    /**
        * Generated a bump map image for the input image
        */
    static vsg::Image* createBumpMap(const vsg::Image* input);
    ...
    /**
        *
        */
    static vsg::Image* upSampleNN(const vsg::Image* src, int quadrant);
    ...
    /**
        * Reads an vsg::Image from the given input stream.
        * Returns NULL if the image could not be read.
        */
    static vsg::Image* readStream(std::istream& stream, const osgDB::Options* options);
    ...
    public:
        /**
            ...
            */
        PixelReader(const vsg::Image* image);

        /** Sets an image to read. */
        void setImage(const vsg::Image* image);
        ...
        static bool supports(const vsg::Image* image)
        {
            return image && supports(image->getPixelFormat(), image->getDataType());
        }

        /** Reads a color from the image */
        vsg::vec4 operator()(int s, int t, int r = 0, int m = 0) const
        {
            return (*_reader)(this, s, t, r, m);
        }

        /** Reads a color from the image */
        vsg::vec4 operator()(unsigned s, unsigned t, unsigned r = 0, int m = 0) const
        {
            return (*_reader)(this, s, t, r, m);
        }

        /** Reads a color from the image by unit coords [0..1] */
        vsg::vec4 operator()(float u, float v, int r = 0, int m = 0) const;
        vsg::vec4 operator()(double u, double v, int r = 0, int m = 0) const;
        ...
        typedef vsg::vec4 (*ReaderFunc)(const PixelReader* ia, int s, int t, int r, int m);
        ReaderFunc _reader;
        const vsg::Image* _image;
        unsigned _colMult;
        unsigned _rowMult;
        unsigned _imageSize;
        bool _normalized;
    ...
    public:
        ...
        void operator()(const vsg::vec4& c, int s, int t, int r = 0, int m = 0)
        {
            (*_writer)(this, c, s, t, r, m);
        }

        void f(const vsg::vec4& c, float s, float t, int r = 0, int m = 0)
        {
            this->operator()(c,
        ...
        typedef void (*WriterFunc)(const PixelWriter* iw, const vsg::vec4& c, int s, int t, int r, int m);
        WriterFunc _writer;
    };
    ...
    struct PixelVisitor : public T {
        ...
        /**
            * Traverse an image, and call this method on the superclass:
            *
            *   bool operator(const vsg::vec4& srcPixel, vsg::vec4& destPixel);
            *
            * If that method returns true, write destPixel back at the same location
            * in the destination image.
            */
        void accept(const vsg::Image* src, vsg::Image* dest)
        {
            ...
            for (int r = 0; r < src->r(); ++r) {
                for (int t = 0; t < src->t(); ++t) {
                    for (int s = 0; s < src->s(); ++s) {
                        const vsg::vec4 pixelSrc = _readerSrc(s, t, r);
                        vsg::vec4 pixelDest = _readerDest(s, t, r);
...
public:
    ...
    virtual void apply(vsg::Image& image) {}
