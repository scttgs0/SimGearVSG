...
/**
     ...
     */
class MethodHolder : public SGWeakReferenced
{
public:
...
/**
   ...
   */
template <class T>
class Ghost<T, std::enable_if_t<is_strong_ref<T>::value>> : public internal::GhostMetadata
{
public:
    ...
    class MethodHolder : public internal::MethodHolder
    {
    public:
        explicit MethodHolder(const method_t& method) : _method(method)
        {
        }
    ...
    struct member_t {
        ...
        member_t(const getter_t& getter,
                 ...
                 const MethodHolderPtr& func = MethodHolderPtr()) : getter(getter),
                                                                    setter(setter),
                                                                    func(func)
        {
        }

        explicit member_t(const MethodHolderPtr& func) : func(func)
        {
        }
                  ...
                  std::index_sequence<Indices...>)
    {
        return method<Ret>(name,
                           typename std::function<Ret(raw_type&, const CallContext&)>(std::bind(func,
                                                                                                std::placeholders::_1,
    ...
    static std::enable_if_t<
        std::is_same<RefType, strong_ref>::value || std::is_same<RefType, weak_ref>::value,
        naRef>
    ...
    static strong_ref fromNasalChecked(naContext c, naRef me)
    {
        ...
        std::string msg = "Can not convert to '" + getSingletonPtr()->_name_strong + "': ";
...
private:
    ...
    struct DerivedInfo {
        ...
        DerivedInfo(to_nasal_t to_nasal_func,
                    from_nasal_t from_nasal_func) : to_nasal(to_nasal_func),
    ...
    static std::enable_if_t<!is_weak, RefPtr>
    getPtr(void* ptr)
    {
    ...
    /**
       ...
       */
    template <class Param>
    static bool convert_param_invoker(
    ...
    /**
       ...
       */
    template <class Ret>
    static std::enable_if_t<!std::is_void<Ret>::value, naRef>
    method_invoker(
        const std::function<Ret(raw_type&, const CallContext&)>& func,
        raw_type& obj,
    ...
    explicit Ghost(const std::string& name) : GhostMetadata(name,
                                                            &_ghost_type_strong,
    ...
    static std::enable_if_t<!is_weak, naRef>
                                     ...
                                     naRef* out)
    {
        ...
        if (member == getSingletonPtr()->_members.end()) {
            ...
            if (!fallback_get || !fallback_get(obj, c, key_str, *out))
                return 0;
...
template <class T>
std::enable_if_t<
    std::is_base_of<SGReferenced, T>::value || std::is_base_of<SGWeakReferenced, T>::value,
    naRef>
to_nasal_helper(naContext c, T* ptr)
{
...
template <class T>
std::enable_if_t<
    std::is_base_of<SGReferenced, std::remove_pointer_t<T>>::value || std::is_base_of<SGWeakReferenced, std::remove_pointer_t<T>>::value,
    T>
from_nasal_helper(naContext c, naRef ref, const T*)
{
...
to_nasal_helper(naContext c, T* ptr)
{
    return nasal::Ghost<vsg::ref_ptr<T>>::makeGhost(c, vsg::ref_ptr<T>(ptr));
}
...
from_nasal_helper(naContext c, naRef ref, const T*)
{
    using TypeRef = vsg::ref_ptr<std::remove_pointer_t<T>>;
    return T(nasal::Ghost<TypeRef>::fromNasalChecked(c, ref));
}
