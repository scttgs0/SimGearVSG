...
//----------------------------------------------------------------------------
EventManager::StampedPropagationPath::StampedPropagationPath() : time(0)
{
}
...
//----------------------------------------------------------------------------
EventManager::EventManager() : _current_click_count(0)
{
}
                                  ...
                                  const EventPropagationPath& path)
{
    ...
    if (mouse_event && mouse_event->type == MouseEvent::DRAG) {
        vsg::vec2 local_pos = mouse_event->client_pos;
                               ...
                               const EventPropagationPath& path)
{
    ...
    if (event->getTime() > _last_click.time + multi_click_timeout)
        ...
        if (_current_click_count > 1) {
            ...
            // been clicked
            if (!checkClickDistance(event->screen_pos, _last_click.pos) || _last_click.button != event->button)
                _current_click_count = 1;
        }
    }
...
//----------------------------------------------------------------------------
bool EventManager::checkClickDistance(const vsg::vec2& pos1,
                                      const vsg::vec2& pos2) const
{
    vsg::vec2 delta = pos1 - pos2;
    return std::fabs(delta.x()) < drag_threshold && std::fabs(delta.y()) < drag_threshold;
}
