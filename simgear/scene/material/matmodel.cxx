      ...
      _range_m(range_m)
{
    ...
    if (_coverage_m2 < 1000) {
        SG_LOG(SG_INPUT, SG_ALERT, "Random object coverage " << _coverage_m2 << " is too small, forcing, to 1000");
        _coverage_m2 = 1000;
    }
...
SGMatModel::load_models(SGPropertyNode* prop_root)
{
    ...
    if (!_models_loaded) {
        for (unsigned int i = 0; i < _paths.size(); i++) {
            vsg::Node* entity = SGModelLib::loadModel(_paths[i], prop_root);
            if (entity != 0) {
                ...
                _models.push_back(new vsg::Node());
            }
        }
    }
    _models_loaded = true;
