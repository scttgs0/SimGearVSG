...
public:
    ...
    CloudShaderGeometry(int vx, int vy, float width, float height, float ts, float ms, float bs, float shade, float ch, float zsc, float af) : varieties_x(vx), varieties_y(vy), top_factor(ts), middle_factor(ms),
    ...
    CloudShaderGeometry(const CloudShaderGeometry& CloudShaderGeometry, const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY) : osg::Drawable(CloudShaderGeometry, copyop) {}

    META_Object(flightgear, CloudShaderGeometry);

    struct CloudSprite {
        CloudSprite(const SGVec3f& p, int tx, int ty, float w, float h) : position(p), texture_index_x(tx), texture_index_y(ty), width(w), height(h)
        {
        }
    ...
    vsg::ref_ptr<osg::Drawable> _geometry;
